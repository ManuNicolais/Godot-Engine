...
@onready var player_model = $Armature  # Modelo del jugador (debe existir este nodo)
...
func _physics_process(delta):
	...
	if active_camera == 1: #CAMARA 3ra PERSONA
		
		# Usamos la dirección de la cámara para mover al personaje
		var cam_transform = camera2_controller.global_transform
		var forward = -cam_transform.basis.z.normalized()  # Eje Z (hacia adelante)
		var right = -cam_transform.basis.x.normalized()  # Eje X (hacia derecha)
		...
		# Rotamos el personaje según la cámara (solo en el eje Y) + 180 grados
		player_model.rotation.y = camera1_controller.global_transform.basis.get_euler().y + deg_to_rad(180)
